...
| Version | Change |
|---------|--------|
| v6.0.0 (2026-08-15) | Identity-gate "cadillac" pass matching locks/security_manager. Migrated onto `zen_target_resolve.jinja` for `discover`/`set` (`scene_set` keeps its own `area_entities` walk — it applies per-role scene positions across a whole area, not a caller-specified target set, so it's not a targeting-core candidate). Full SESE + `envelope()` conversion. New `cover_control` cert (asymmetric: close is cert-only, barrier-open is cert+live-ack, non-barrier-open is cert-only). `cert_scope` per-target ack override. Self-publishes `certs_required` for the live cert catalog. Fixed a `dry_run` preview bug that was blind to a granted `cert_scope` override — a dry-run would report "would require ack" for a target actually exempted. Also fixed a real pre-existing bug, inherited from the v5.1.0 baseline, unrelated to this pass but caught while building the barrier gate: `position`/`tilt_position` used Jinja's `default(-1, true)`, which substitutes on *any* falsy value, not just undefined — so `mode=set position=0` (a legitimate "close the cover" call) was silently coerced to the "no position given" sentinel for every caller. Fixed with explicit `is not defined`/`is none` checks. |
