...
| Version | Change |
|---------|--------|
| v6.2.0 | **CabCeption.** Nested drawer trees via `/` path separator. `get` returns `subdrawers`+`has_children` when children exist (additive). `move`/`delete`/`copy` cascade to all children via prefix scan. `move` child writes verified before source delete. `copy` cascades subtree. `inspect` surfaces subdrawers inline. |
