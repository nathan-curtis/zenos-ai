...
| `recipes_parse_ingredients` | Runs Mealie's ingredient parser (`nlp`/`brute`/`openai`) over any freetext-only ingredient lines (quantity 0.0, unit null, food null — common on URL-scraped recipes) and writes back structured quantity/unit/food. Already-structured ingredients are left untouched. Requires `recipe_id` or `recipe`. Run this before `recipe_fulfillment` if a scraped recipe's ingredients aren't checking against stock. Response flags `needs_package_size` for any brand-new Grocy product it just created for a measured unit (gram/teaspoon/etc — not a bare count) — the same gap `recipe_consume` would otherwise hit `needs_manual_check` on later. Worth asking "what size do you buy this in?" once here, at creation time, rather than mid-cook. |
