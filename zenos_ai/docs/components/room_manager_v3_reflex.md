...
**Version:** rev 6 (2026-08-24) — `room_signal_fire` (evidence-simulating manual mode), `presence_hold` tier, `entertaining_cascade_enable`/`_check`, cascade redesign (child-engaged retired, child_emergency/child_paused cascade unconditionally), `room_control_override` restored as its own distinct unpause cert, per-state sensor icons, real fixes to the wasp self-latch/`asleep_hold`/`child_release` and several `room_control_set` race conditions. Same-day follow-ups: `asleep_hold` extended to the redundant-timer "held for any reason" guard in `zen_room_manager_dispatch.yaml`; a bounded 8s wall-clock bridge on `_child_occ_hi` closes the dark-flash-then-scene gap between a child's real state change and `child_release`'s compensating burnout timer; new `child_cascade_arm` trigger/action pair (mirrors `child_release`) cancels a grandparent's own stale local `room_timer` the moment a child starts cascade-sustaining it, same redundant-timer class as wasp/asleep_hold, single-hop by construction so it stays portable to any cascade depth. Previous: rev 5 (2026-08-08) — `room_control_manager` isolation, wasp-hold rewrite, entertaining/guest hold, asleep window
...
consolidation history), see [Architecture Ch. 24](../architecture/24_room_manager_v3.md).
...
1. **Declare helpers in the room's package file** (only what you want — every one of these is optional):
   ...
   **`room_control_manager` (the manual-override select) needs its own
   ...
   (Architecture §24.7) enforced in one place instead of every caller having to know it.
...
| `wasp_door` | A door contact sensor | Real wasp-in-a-box, level-based (no latch, no timer) — see [Architecture §24.7](../architecture/24_room_manager_v3.md) for the full formula. Motion/occupancy live AND every `wasp_door` for the room reads closed → `hold`, recomputed fresh on every render; any `wasp_door` opening clears it that same render, no memory of the last edge. `checking`/`checking_timer` no longer exist. Requires the room-level `wasp_enabled` gate below — `wasp_door` alone is not sufficient. |
...
| `vibration_completion` | Any `vibration`-labeled entity, opt-in | Enables the anomaly-gated "load complete" detector (`zen_room_manager_vibration_completion.yaml`) — 20-minute minimum sustained-run duration before a completion is considered genuine, mirrors `washing_machine.yaml`'s real cycle-duration gate. Emits `zen_dojotools_event_emitter` (`kind: vibration_load_complete`) on genuine completion. State tracked per-entity in the household cabinet drawer `vibration_tracking`, not a dedicated helper — nothing to create beyond the label. **Only entry in this table that needs no room_state sensor at all** — standalone, works pre-Step-3. |
| `ext_lock` | Exterior lock entities | Live read, no dispatch/timer involved — `room_state.yaml` L388/853-855 computes `ext_unlocked_active` (bool), `ext_unlocked_count`, `ext_lock_count` directly off `label_entities('ext_lock') \| intersect(_room_ents)` on every render. Act on a room's locks via `zen_dojotools_locks` (new, v1.0.0) — `mode=set room=<room> action=lock\|unlock` targets every lock in an area via `area_entities()`, no label required for the action tool itself (only the room_state aggregate needs the label). |
| ~~`privacy_door`~~ | — | **Dead, as of 2026-08-15.** Used to try to `input_boolean.turn_on/off` the triggering lock entity itself (wrong domain, silently failed) via `zen_room_manager_dispatch.yaml` L138-142 — removed rather than left broken. `room_state.yaml` never consumed it either. If you find this label on a real entity, retag it `ext_lock`. |
...
| `room_timer` + `room_timer_class` | Timer + paired select | Enables decay-backed occupied/engaged/asleep — one shared clock for all three. `asleep` class default is 8h (was 30m). **Stale-class self-heal:** a plain hold-release deliberately doesn't restart an `asleep` timer, which can leave the stored `room_timer_class` stuck on `asleep` with the timer cancelled; the motion and door arm gates would then silently never re-arm. On the next motion/door event, if the room isn't wasp-latching, isn't genuinely held/asleep per its **live** state sensor, and its timer is idle, the dispatcher resets the stale class to `''` first, so that same event arms normally. |
...
- [Architecture Ch. 24: Room Manager v3 Reference](../architecture/24_room_manager_v3.md) — full design rationale, the consolidation history, and the RoomState/RoomReg boundary
...
- [Live State](../architecture/15_live_state.md): the concepts this system implements
