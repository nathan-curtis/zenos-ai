...
- **`room_topology_edit`** — structural edits: `set`, `setup`, `area_create`, `area_update`, `link`, `unlink`, `boundary_link`, `boundary_unlink`, `zone_set`, `zone_remove`, `landmark_set`, `landmark_remove` (level 1). `area_delete` requires level 2 — the existing per-cert graduated `max_level` mechanism, not a third cert class.
