...
There is no per-component cooldown timer and no quiet-hours policy in the Scheduler. Queue-depth shedding is what prevents storms. The per-component emission cooldown (Chapter 13) governs escalation, not summarization.
...
| `zen_scheduler_drain_router` | Recovers shed components (21.3) |
...
Daily, and on Home Assistant start, `zen_dojotools_manifest` also re-runs its bootstrap modes: KFC self-registration and Lens Bus provider registration (Chapter 12).
...
[← Identity as Traversal](20_identity_as_traversal.md) · [Contents](00_toc.md) · [Resilience →](22_resilience.md)
<!-- /nav -->
