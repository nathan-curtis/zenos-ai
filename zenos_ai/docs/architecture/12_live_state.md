...
This chapter describes what a room's state means and how it is computed. Chapter 21 is the operational reference: every label, every construct, every edge case.

## 12.1 One state per room
...
A room's state is what an agent means when it asks whether a room is occupied. It is also what REFLEX acts on (12.4).

## 12.2 Signals come from labels
...
This is the hypergraph again (Chapter 3). A motion sensor carries `motion` and its room's label. The intersection of those two hyperedges is the whole routing table.

## 12.3 Time, children, and explanation
...
## 12.4 REFLEX: state becomes effect
