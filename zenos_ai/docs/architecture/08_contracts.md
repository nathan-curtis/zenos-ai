...
| Field | Meaning |
|---|---|
| `tool`, `display_name`, `tier` | Identity and class (Chapter 9) |
| `version` | The tool's version. This is the canonical version of the tool (8.4). |
| `health` | Whether the tool is configured and working, with notes |
| `required_labels`, `optional_labels` | The vocabulary it depends on (Chapter 7) |
| `missing_required_labels`, `missing_optional_labels` | Computed on every call against the live label registry |
| `certs_required` | The certifications it enforces, per mode and level (Chapter 15) |
