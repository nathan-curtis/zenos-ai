...
## 1.1 The substrate is local. Inference is pluggable.
...
## 1.2 Structure around stochastic inference.
...
What ZenOS makes deterministic is everything around the model: what goes in, what shape must come back, and what happens when the shape is wrong. A summarizer builds its prompt from declared sources, asks for a declared schema, and validates the result before writing it. A response that does not parse into a real, non-empty object is logged and dropped. It is never written as if it were valid (see Chapter 11). The model is allowed to be probabilistic. The pipeline is not.

## 1.3 The graph is the ground truth. The model holds nothing.
...
This is not only a context-budget decision, though it is one. A tool can enforce what raw entity exposure cannot: target resolution, preview before actuation, certification checks, and a consistent response shape. Chapter 6 covers it in full.

## 1.5 One exit, one shape.
...
`status` answers whether the call itself succeeded. `result` carries the tool's own answer. A caller never has to guess which of a dozen shapes it got back, and a tool cannot leave early along a path that skipped its own checks. Chapter 8 lists the contract, and the tools that do not yet follow it.

## 1.6 Fail loud. Fail closed.
...
## 1.7 A human is the last gate on what matters.
...
## 1.8 Declare what you are. Say what you are not.
