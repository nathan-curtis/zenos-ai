...
## 6.1 Two ways to reach the house
...
## 6.3 How an agent works with zero entities
...
At no point does the agent need an entity exposed. The tools resolve every target from labels (Chapter 3).
...
AdminTools stay off the agent surface unless a human deliberately exposes one for recovery (Chapter 16). Each tool's manifest declares whether it is meant to be exposed (`mcp_exposed`), and the getting-started guide carries the current exposure list.
