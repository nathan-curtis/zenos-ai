...
| `suggested_act_desc`, `suggested_act_event` | What, and which tool call would do it (Chapter 10) |
...
`home_overview` is the coarse situation: overall status, active risks, recent notable events. `active_components` is the per-domain view of whatever SuperSummary decided to include (Chapter 10). The `zen_summary` block is the synthesis: what matters, what should happen, how urgent it is, and how sure the system is.

## 11.3 The write guard
...
Consumers decide whether a Kata is current from its own fields: `last_run_at` on a component Kata, `timestamp` and `stale_after` on `zen_summary`. Each KFC declares a `staleness_minutes` ceiling. A component whose Kata passes that ceiling is dispatched for a refresh even when the scheduler would otherwise shed it under load (Chapter 18).
