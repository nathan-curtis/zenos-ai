...
- **`room_timer_class`** (the internal tag tracking which decay timer a room is running) moved off a dedicated per-room helper entity onto the room's own `room_topology` cabinet entry — no functional change, just one less helper entity per room.
- **A real Paused-safety race, closed.** `room_control` now lives in its own atomic per-room cabinet key, separate from `room_topology` — previously, a concurrent write elsewhere in the dispatcher could occasionally race a Paused write and silently restore the room's prior state, undoing an intentional safety stop. Not reproducible anymore; there's no longer a shared drawer for the two writes to race against.
- **Cross-room shared safety sensors now latch every room they cover.** One smoke or CO detector labeled for multiple adjacent rooms (a documented, supported pattern) previously only latched the first matched room's emergency state — every room sharing that sensor now correctly goes to Emergency together.
- **`autosleep_schedule` is now honored consistently everywhere.** A room with a non-standard sleep schedule (a shift worker, say) was correctly handled by the room's own state sensor, but the dispatcher's separate timer-arming logic didn't know to check for it and could still arm against the house's normal clock — both paths now agree.
...
**Status:** Merged to `main` locally, pending push + tag
