...
| `dojotools/dojotools_plant.yaml` | v5.4.0. Physical plant + energy manager. v1.3.1: mode=thermal, mode=mechanical water_management subnode, mode=ignore/unignore. v1.5.0: motors[], include_inventory, water_management rename, name+area on all load nodes. v5.3.0: mode=managed (provision_bom machine rollup, chore dedup, location→area resolution, advisory[]). v5.4.0: all block templates | tojson + | from_json at use-sites (fixes Python repr leakage); water usage max(0) clamp; motors/sump structured nulls; gas "not an error" messaging. |
...
| `plugins/grocy/grocy.yaml` | v5.2.0. v4.54: chores_delete/edit, unit_conversions, product_groups, update_product_meta full RMW, null-unit guard. v4.48: `room_brief` (three-path chore discovery), `chores_tag`, `tasks_add/tag`, `slim_objects`, product_name enrichment. v4.49–v4.54: ERP object substrate — userentities/userobjects/userfields CRUD. v5.x: `provision_bom` (unit resolve, product create/find, meta, labels, chore, stock seed); 3-tier product resolution (caller-pin → exact name → same-location+ha_labels-root fallback); null-unit guard in stock_buy_product/stock_add_purchase; limit=500 on unit fetch. 96 operations. |
...
| `zenos_ai/docs/plugins/grocy.md` | v5.2.0: updated for provision_bom, 3-tier resolution, null-unit guard |
