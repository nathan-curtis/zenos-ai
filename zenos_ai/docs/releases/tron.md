...
**Still open, tracked for RC, not a beta blocker:** FileCabinet has no cert-gate at all yet — deliberately held pending a design decision on its boot-dependency problem, not an oversight. Manifest and Scribe got the cadillac SESE/envelope treatment this cycle but haven't had a dedicated cert-gate audit pass the way the rest of the platform did. Postman closed its own audit as a follow-up (`zenos.comms.dispatch` now gates send/delivery-delete/whitelist-edit). Both remaining tools need to close before this becomes a release candidate.
