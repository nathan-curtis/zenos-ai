...
**What's in 2026.9.0 'Steel Magnolia' (RC1):** *The duck looks calm, and the fence is exactly where it's supposed to be.* Room Manager v3 — a room that reads what's actually happening in the space right now instead of watching a clock, with a real wasp-in-a-box model, plus opt-in `entertaining_hold`/`guest_hold` and a nighttime-gated asleep window; Asleep now correctly outranks Engaged in the priority cascade. Two new canonical domain tools, `zen_dojotools_boolean` and `zen_dojotools_timekeeper` (timer GET+SET plus dashboard/worldclock/alarms rollups). ZenLux and Plant Manager both gain Room Manager v3 awareness (`reflex_sync`, a room-lock guard, `label_suggest` for SPAN/Emporia circuits). ZQ-1 query validation surfaces bad `filter_json` keys instead of silently returning nothing. Recorder history stats stop lying about energy sensors (state-class-aware bucket types, `period_total_24h`). Architecture docs checked chapter-by-chapter against actual code. New identity gates on locks, exterior covers, the alarm panel, room unpause/topology edits, ZenLux, climate, and the spa tool — risky actuations now require an explicit household certification plus, for the highest-risk actions, a fresh live yes/no every time — closing a real self-escalation hole found while building it, with per-tool scope resolution centralized to one shared macro instead of five separate copies and a real deny primitive (not just allow-scoping). A full audit for gate end-runs found and closed two real gaps: ZenZork's text-adventure commands and the spa tool were both actuating real hardware with zero identity check. ZenLux also gains real `switch.*` control, not just lights; climate inherits general-purpose `fan.*` control, a primitive that didn't exist before. ZenZork ships its first real content chapter and a v1.8.0 follow-up — weighted loot, 16 quests, a corrected book-lore sequence, Game Genie cheat codes, a SoftDisk-style per-chapter release model, real persisted achievements, and a turn-based threat/combat system. A new admin-only CertAdmin tool centralizes cert authority and adds a per-cert live-ack waiver; fixed a compliance blind spot where audit modes had silently excluded the whole admintools tier since 2026-07-29; `sensor.home_overview` now feeds Friday's prompt real per-room state instead of an empty object. Hospitality lifecycle gains an arrival-prep nudge and a checkout nudge (both with humanized local timestamps) alongside a real UTC/local storage bug fix; the dispatcher's `urgency_handler` is a real implementation now instead of a silent no-op; five of manifest's scan modes (including a new declared-dependency `toolmap`) share one collector instead of five hand-rolled fan-out loops. A late Room Manager batch lands on top: hold-release now restarts a room's suspended timer instead of snapping to Vacant, a new generic `trouble` class (plus native SPAN circuit-overload detection) surfaces electrical/safety signals as room attributes, a wasp blind-period cooldown stops a quick door cycle from re-latching a false lock, and Paused/Emergency now cascade through nested rooms unconditionally. A mid-cycle Roborock hardware swap surfaced a batch of real AutoVac bugs, all now fixed (battery, wear/consumables, room targeting, error handling, manual-run tracking), plus a severe one: the DND election gate had silently blocked every automatic cleaning election since it shipped. Also fixed: a real essence-corruption bug that could silently reset an AI persona's identity on bootstrap, a Twenty CRM date bug that kept stale guest stays reporting as active indefinitely, and a self-sustaining alert loop between two summarizer components that had been alarming about each other's own tickets since mid-August. Security Manager gets a full parity build against the legacy automation it replaces (new arm/disarm/mode-drive flags, a vacation wake-shift, a real FileCabinet write bug that had silently no-op'd every flag flip) plus a sleep-gate bypass for live-ack requests so a Paused-clear or cert grant never silently waits for morning. ZenLux picks up Spook 5.2's adjust-only light controls and REFLEX now fires scenes through it instead of bypassing its guards — a pass that also caught REFLEX's dry-run mode silently stuck on house-wide, meaning it had been resolving correctly but never actually firing. SPAN circuit sensors now confirm a breaker actually tripped (not just predict one) and feed that into Room Manager's `trouble` signal. Also fixed: two real ZenZork bugs (player identity resets, locked doors not blocking traversal), a Mealie search gap for tag-only terms, and agent health no longer following the summarizer pipeline's health into a false `warn`.
